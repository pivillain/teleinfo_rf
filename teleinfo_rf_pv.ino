...
boolean handleBuffer(char *bufferTeleinfo, int sequenceNumnber)
{
  // create a pointer to the first char after the space
    ...
    Label=Buffer.substr(0,(Buffer.length()-(Buffer.indexOf(' ')-1));
    Value=Buffer.substr(Buffer.indexOf(' ')+1);

    if (Label.equalsIgnoreCase("ADCO")){ADCO=Value;}
    if (Label.equalsIgnoreCase("OPTARIF")){OPTARIF=Value;}
    if (Label.equalsIgnoreCase("ISOUSC")){ISOUSC=atoi(Value);}
    if (Label.equalsIgnoreCase("BASE")){}
    if (Label.equalsIgnoreCase("HCHC")){HCHC=atol(Value);}
    if (Label.equalsIgnoreCase("HCHP")){HCHP=atol(Value);}
    ...
    if (Label.equalsIgnoreCase("PTEC")){PTEC=Value;}
    if (Label.equalsIgnoreCase("DEMAIN")){}
    if (Label.equalsIgnoreCase("IINST")){IINST=atoi(Value);}
    if (Label.equalsIgnoreCase("ADPS")){}
    if (Label.equalsIgnoreCase("IMAX")){IMAX=atoi(Value);}
    if (Label.equalsIgnoreCase("PAPP")){PAPP=atoi(Value);}
    if (Label.equalsIgnoreCase("HHPHC")){HHPHC=Value;}
...
//strcpy(&bufferTeleinfo[0],"PMAX 06030");
//handleBuffer(bufferTeleinfo, 9);
...
//fin test debug //teleInfoReceived=readTeleInfo(true);
if (teleInfoReceived)
  {
  encodeur_OWL_CM180();
